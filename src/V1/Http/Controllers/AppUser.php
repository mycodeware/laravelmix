...
class AppUser extends BaseController
{
    ...
    public function checkLogin(Request $request)
    {
        ...
        $credentials = (['email' => $params['email'], 'password' => $params['password'], 'user_status_id' => 1]);
...
$user = User::where('email', '=',$params['email'])->first();
     ...
     else
     {
        ...
        $params['otp_status'] = "sent"; 

        $params['super_store_id'] = getenv('SuperStoreId');
        ...
        $params['otp_validity'] = $validfor ; 

					$EmailOtpMapping = new EmailOtpMapping();

					$EmailOtpMapping->email = $params['email'];
					$EmailOtpMapping->super_store_id = $params['super_store_id'];
					$EmailOtpMapping->otp = $params['otp'];
					$EmailOtpMapping->otp_validity = $params['otp_validity'];
					$EmailOtpMapping->otp_status = $params['otp_status'];

					$EmailOtpMapping->save();
        ...
        $flag = EmailOtpMapping::where('email', '=',$params['email'])
...
$this->logger->error(Utils::json($payload), $errorResponse);
            ...
            exit();
                ...
                $userinfo=User::where('email',$params['email'])->update(['password'=>$pass]);
        ...
        {               
                        ...
                        $user = User::where('email', '=',$params['email'])->where('login_social_platform_id','=',$params['social_platfrom_id'])->where('social_media_user_id','=',$params['socialAccountId'])->first();
                        ...
                        $params['otp'] = $otp; 
                        $params['otp_status'] = $this->otp_status; 
                                               ...
                                               $loginuser = Auth::login($user);
                                                ...
                                                if(Auth::user()->id)
                                                {
                                                     $data = [
                                                          ...
                                                          $this->deviceToken(getenv('superStoreId'),Auth::user()->user_code,'staticToken','deviceSerial', $platform=$params['locale']['platform']);
                         ...
                         $messages = [
                          ...
                          $rule = [
                             ...
                             $validator = Validator::make($params, $rule, $messages);
                                ...
                                $userinfo=User::where('email',$params['email'])->update(['user_status_id'=>1]);
