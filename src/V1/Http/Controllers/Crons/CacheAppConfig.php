 ...
 * DB Models
 */
use Jet\Publicam\JetEngage\V1\Models\Sql\Configuration;
use Jet\Publicam\JetEngage\V1\Models\Sql\StoreLoginSocialPlatformMapping;
...
{
    protected $platforms = ['android','ios','androidtv','iostv','samsungtizen','amazonfire'];

    protected $platformsTv = ['androidtv','iostv','samsungtizen','amazonfire'];
        ...
        $configurations = Configuration::where('package_name', $packageName)
            ...
            if (!empty($configurations)) {
                ...
                foreach ($this->platforms as $platform) {
                    ...
                    if(in_array($platform, $this->platformsTv)) {
                        $facebookApp = [
                            'AppId' => env('FACEBOOK_APP_ID_TV', ''),
                            'Secret' => env('FACEBOOK_APP_SECRET_TV', ''),
                            'ClientToken' => env('FACEBOOK_APP_CLIENT_TOKEN_TV', ''),
                        ];
                    } else {
                        $facebookApp = [
                    ...
                    if (isset($splashAd) && !empty($splashAd)) {
                        $response['data']['splashPoweredBy'] = $splashAd;
                        unset($splashAd);
                    }
