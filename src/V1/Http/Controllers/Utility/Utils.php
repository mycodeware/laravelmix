    ...
    */
    ...
    public static function validateLocale(array $params) : array
    {
        ...
        if (
            ...
            strtolower($params['locale']['platform']) != 'androidtv' &&
            strtolower($params['locale']['platform']) != 'appletv' &&
            strtolower($params['locale']['platform']) != 'tizen'
        ...
        */
        ...
        if(! in_array($params['locale']['platform'],config('v1.appPlatforms'))) {
            return [
                "code" => 601,
                "status" => "failed",
                "message" => "Invalid Platform."
     ...
     * Generate User Code
     */
    public static function getUserCode(int $id, int $ssid) : string
    {
        return $ssid.'-'.$id.'-'.time();
    }
     ...
     * @return array
     */
    public static function getContentEngagement(array $params, array $contentIds)
    {
        $contentEngagement = [];
        $contentLike = '0';
        $contentShare = '0';
        ...
        if (isset($params['userCode']) && $params['userCode'] !== "") {
            $checkContentLike = \Jet\Publicam\JetEngage\V1\Models\Sql\ContentEngagement::whereIn('content_id', $contentIds)

            ->groupBy('content_id')
            ->where([
                'user_code' => $params['userCode'],
                'super_store_id' => $params['superStoreId']+0,
        ...
        $contentCounts = \Jet\Publicam\JetEngage\V1\Models\Sql\ContentEngagementCount::whereIn('content_id', $contentIds)
        ...
        if (! $contentCounts->isEmpty()) {
            foreach ($contentCounts as $id => $contentData) {
                $contentEngagement[$contentData['content_id']]['like_count'] = Utils::restyleText($contentData->like_count);
                $contentEngagement[$contentData['content_id']]['share_count'] = Utils::restyleText($contentData->share_count);
                $contentEngagement[$contentData['content_id']]['comment_count'] = Utils::restyleText($contentData->comment_count);
                $contentEngagement[$contentData['content_id']]['view_count'] = Utils::restyleText($contentData->view_count);

                if (isset($checkContentLike) &&
                    ! empty($checkContentLike) &&
                    Utils::in_array_r($contentData['content_id'], $checkContentLike, true)
                ) {
                    $contentEngagement[$contentData['content_id']]['isLiked'] = 1;
                } else {
                    $contentEngagement[$contentData['content_id']]['isLiked'] = 0;
                }
            }
        }
